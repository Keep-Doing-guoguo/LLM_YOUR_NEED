...
if __name__ == "__main__":
    # —— 与脚本: MODEL="Qwen/Qwen-7B"
    model_args = ModelArguments(
        model_name_or_path="Qwen/Qwen-7B"
    )

    # —— 与脚本: DATA="path_to_data"
    data_args = DataArguments(
        data_path="path_to_data",
        eval_data_path=None,     # evaluation_strategy="no"，不做验证
    ...
    training_args = TrainingArguments(
        ...
        optim="adamw_torch",                      # 默认与脚本一致

        bf16=True,                                # --bf16 True
        fp16=False,                               # 脚本未启用 fp16
        ...
        deepspeed="finetune/ds_config_zero3.json" # --deepspeed finetune/ds_config_zero3.json
    )

    # 脚本未启用 LoRA/QLoRA，保持默认关闭
    lora_args = LoraArguments(
        # 保持默认值；若后续需要可开启：
        # use_lora=True, q_lora=False, ...
    )
...
或者：torchrun --nproc_per_node 1 train.py
单机多卡
...
把 MASTER_ADDR/PORT 换成你实际的 Rank0 节点信息。
...
有效批大小 ≈ per_device_train_batch_size × n_gpus × gradient_accumulation_steps。上面 8 卡时为 1 × 8 × 16 = 128。
