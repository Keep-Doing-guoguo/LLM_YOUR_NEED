...
@author: zgw
@date: 2025/4/7 16:16
@source from: https://blog.csdn.net/xzq_qzx_/article/details/136525979
"""
import re
...
if __name__ == "__main__":
    ...
    llm = ChatOpenAI(
        ...
        openai_api_key="你的key",  # 替换为你的 key
    ...
    response = agent_executor.run("什么是Rimix？")
    print('debug')
    # while True:
    #     try:
...
查询产品名称: 通过产品名称找到产品描述时用的工具，输入应该是产品名称
复歌科技公司相关信息: 当用户询问公司相关的问题，可以通过这个工具了解相关信息
回答时需要遵循以下用---括起来的格式：
---
Question: 我需要回答的问题
Thought: 回答这个上述我需要做些什么
...
---
Question: 什么是Rimix？  
Thought: 要回答这个问题，我需要查询Rimix的产品描述以了解它是什么。  
Action: 查询产品名称  
Action Input: Rimix  

Observation:Rimix通过采购流程数字化、完备的项目数据存储记录及标准的供应商管理体系，帮助企业实现采购流程, 透明合规可追溯，大幅节约采购成本。Rimix已为包括联合利华，滴滴出行等多家广告主提供服务，平均可为客户节约采购成本30%。
按照给定的格式回答以下问题。你可以使用下面这些工具：
查询产品名称: 通过产品名称找到产品描述时用的工具，输入应该是产品名称
复歌科技公司相关信息: 当用户询问公司相关的问题，可以通过这个工具了解相关信息
回答时需要遵循以下用---括起来的格式：
...
我现在知道最终答案。  
Final Answer: Rimix是一款通过采购流程数字化、完备的项目数据存储记录及标准的供应商管理体系，帮助企业实现采购流程透明合规可追溯并大幅节约采购成本的工具。Rimix已为包括联合利华、滴滴出行等多家广告主提供服务，平均可为客户节约采购成本30%。

> Finished chain.
Rimix是一款通过采购流程数字化、完备的项目数据存储记录及标准的供应商管理体系，帮助企业实现采购流程透明合规可追溯并大 
幅节约采购成本的工具。Rimix已为包括联合利华、滴滴出行等多家广告主提供服务，平均可为客户节约采购成本30%。 
----------------------------------------------------------------
请输入您的问题：
