...
if __name__ == "__main__":
    model_args = ModelArguments(
        model_name_or_path="Qwen/Qwen-7B-Chat-Int4"   # 与脚本 MODEL 一致（GPTQ-Int4）
    )

    data_args = DataArguments(
        data_path="path_to_data",       # 与脚本 --data_path 一致
        eval_data_path=None,            # 训练中不评估
    ...
    training_args = TrainingArguments(
        ...
        bf16=False,                     # 脚本要求 QLoRA+GPTQ 用 fp16
        fp16=True,                      # --fp16 True
        ...
        deepspeed="finetune/ds_config_zero2.json"  # --deepspeed ...
    )

    lora_args = LoraArguments(
        # 脚本仅开启 --use_lora/--q_lora，未给细粒度超参：保持你原设置
        lora_r=8,
        lora_alpha=32,
        lora_dropout=0.05,
