# 一键将VOC数据集转换为YOLO格式，附完整Python脚本解析

在使用 YOLO 系列模型训练时，常常需要将 Pascal VOC 数据集的 XML 标注格式转换为 YOLO 所需的 TXT 格式。本篇文章提供一个完整的 Python 脚本，实现以下功能：

- 将 VOC 的 `.xml` 标注文件转换为 YOLO 格式的 `.txt`
- 将图像文件复制到目标目录
- 根据 JSON 标签生成 `.names` 类别标签文件
...
│   ├── JPEGImages/        # 图片文件
│   └── ImageSets/Main/    # 训练/验证文件列表（train.txt、val.txt）
```
...
# voc_version = "VOC2012"
voc_root = "/Volumes/PSSD/sources/VOCdevkit2"
voc_version = "VOC2007"
# 转换的训练集以及验证集对应txt文件
train_txt = "train.txt"
val_txt = "val.txt"
...
```
## 代码说明：下面的代码可以通过添加上面的划分完毕后的数据路径转变为train.txt和val.txt.
```python
#!/usr/bin/env python
# coding=utf-8
...
def generate_txt(image_dir, output_txt):
...
generate_txt('my_yolo_dataset/train/images', 'train.txt')
generate_txt('my_yolo_dataset/val/images', 'val.txt')
```
